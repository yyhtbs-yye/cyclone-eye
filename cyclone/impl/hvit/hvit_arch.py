...
from cyclone.impl.hvit.hvit_bricks import VITBlock
from cyclone.impl.swin.swin_bricks import STLBlock, PostConv
...
class HVIT(nn.Module):
    def __init__(self, in_chans=3, out_chans=3, 
                 ...
                 window_size=8, qkv_bias=True, qk_scale=None,
                 mlp_ratio=4.0, drop=0.0, attn_drop=0.0, norm_layer=ChannelLayerNorm2d):
        super().__init__()

        pre_block_def = None

        main_block_defs = [(STLBlock, dict(num_heads=num_heads, window_size=window_size, shift_size=0, 
                                           mlp_ratio=mlp_ratio,
                                           qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop, attn_drop=attn_drop,
                                           norm_layer=norm_layer)),
                           (VITBlock, dict(num_heads=num_heads, patch_size=window_size, 
                                          mlp_ratio=mlp_ratio, 
                                          qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop, attn_drop=attn_drop,
                                          norm_layer=norm_layer)),]

        post_block_def = (PostConv,  dict())
